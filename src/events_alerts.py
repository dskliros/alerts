...
def make_subject(event_count):
    ...
    return f"Daily Events Feed - {event_count} Hot Event{'s' if event_count != 1 else ''} Found"

def make_plain_text(df, run_time):
    """Generate plain text version of email"""
    if df.empty:
        return f"""Daily Events - {run_time.strftime('%Y-%m-%d %H:%M %Z')}
...
"""
    ...
    text = f"""Daily Events - {run_time.strftime('%Y-%m-%d %H:%M %Z')}

Found {len(df)} event(s) matching criteria:
- Type ID: {EVENT_TYPE_ID}
...
def make_html(df, run_time, has_logo=False):
    """Generate professional HTML email with results table"""
...
<body>
    <div class="header">
        <div class="logo">
            {logo_html}
            <h1>{COMPANY_NAME} - Events Report</h1>
        </div>
    </div>
    
    <div class="metadata">
        <strong>Report Generated:</strong> {run_time.strftime('%A, %B %d, %Y at %H:%M %Z')}<br>
        <strong>Query Criteria:</strong> Type ID = {EVENT_TYPE_ID}, Name contains "{EVENT_NAME_FILTER}", Last {EVENT_LOOKBACK_DAYS} days<br>
